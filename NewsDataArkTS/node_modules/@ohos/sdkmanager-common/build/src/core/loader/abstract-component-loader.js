...
class AbstractComponentLoader {
    ...
    isValidComponent(component) {
        ...
        if (!component_contants_1.VERSION_PATTERN.test(component.getVersion())) {
            this.progress.warn(`Illegal component ${component.getDisplayName()}:${component.getVersion()}, version is incorrect`);
            return false;
        }
        if (component.getApiVersion() === undefined || component.getApiVersion() < 0) {
            this.progress.warn(`Illegal component ${component.getDisplayName()}:${component.getVersion()}, 
        apiVersion should not be less than 0`);
            return false;
        }
        if ((0, utils_1.isEmpty)(component.getLocation())) {
            this.progress.warn(`Illegal component ${component.getDisplayName()}:${component.getVersion()}, location is null`);
            return false;
        }
        return true;
    }
