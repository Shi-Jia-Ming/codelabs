...
class SdkInfoConfig {
    ...
    static create(sdkRoot) {
        if ((0, utils_1.isEmpty)(sdkRoot)) {
            throw new sdk_exception_1.SdkException('sdkRoot should not be empty', new error_tip_1.ErrorTip('', '', ''));
        }
        const validationResult = (0, sdk_validator_util_1.validateSdkPath)(sdkRoot);
        if (validationResult.status === validator_1.Status.ERROR) {
            throw new sdk_exception_1.SdkException(validationResult.message, new error_tip_1.ErrorTip('', '', ''));
        }
