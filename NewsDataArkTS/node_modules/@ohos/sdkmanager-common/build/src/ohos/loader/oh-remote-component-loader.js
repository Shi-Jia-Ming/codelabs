...
class OhRemoteComponentLoader extends abstract_component_loader_1.AbstractComponentLoader {
    constructor(settings, axiosInstance, progress = new simple_progress_1.SimpleProgress()) {
        super(progress);
        this.load = () => __awaiter(this, void 0, void 0, function* () {
            const { _settings, _axiosInstance } = this;
            try {
                const grsService = yield _settings.getGrsService();
                if (!grsService) {
                ...
                const response = yield _axiosInstance.post(queryMetaUrl, {
                    osType: _settings.getOsType(),
                    supportVersion: oh_component_constants_1.SUPPORT_VERSION,
                });
                const resultData = this.configComponent(response.data);
