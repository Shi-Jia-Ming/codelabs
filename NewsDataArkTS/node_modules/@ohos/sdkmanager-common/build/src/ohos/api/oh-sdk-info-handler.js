...
class OhSdkInfoHandler extends common_sdk_info_handler_1.CommonSdkInfoHandler {
    ...
    getOrDownload(pathAndApiVersions) {
        ...
        return this.doGetOrDownload(pathAndApiVersions, this.mapComponents(loader.load()), component => {
            const paths = this._getLocalIncompatibleSdks(loader);
            if (paths.includes(component.getPath())) {
                throw new sdk_exception_1.SdkException('', new error_tip_1.ErrorTip('', 'The way the SDK is managed has changed.', 'Go to File > Settings > SDKs > OpenHarmony to handle the change'));
            }
        });
    }
    getLocalComponents() {
    ...
    findUnacceptedLicenses(components) {
        ...
        return new oh_license_manager_1.OhLicenseManager(this.sdkSettings, this.sdkSettings.getNetClient())
            .findUnacceptedLicenses(licenseIds, this._progress);
    }
    getLicenseNotAcceptedMessage(fullNameList) {
        const sdkInfo = fullNameList.join(',');
        return new error_tip_1.ErrorTip('', 'The OpenHarmony SDK license agreement is not accepted.', `Go to File > Settings > SDKs > OpenHarmony, download the ${sdkInfo} SDK, and accept the license agreement.`);
    }
