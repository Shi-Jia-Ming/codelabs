# NewsDemoEts_HOS
简介
• 本案例基于tabs和tableContent组件实现了新闻列表的效果，通过http数据请求实现新闻数据的加载，使用touch事件实现新闻数据的下拉刷新和上拉加载效果。
...
What is it?
In this case, the news list is implemented based on tabs and tableContent components. The news data is loaded through HTTP data requests. The touch event is used to implement the pull-down refresh and pull-up loading of news data.
