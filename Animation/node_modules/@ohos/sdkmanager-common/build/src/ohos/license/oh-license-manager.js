...
class OhLicenseManager extends license_manager_1.AbstractLicenseManager {
    constructor(sdkSettings, axiosInstance) {
        super();
    ...
    getReadLocalHashErrorMessage(licensesLocation) {
      ...
      go to Tools > SDK Manager > SDKs > OpenHarmony to download any SDK. Open SDK Manager`).getMessage();
    }
    retrieveLicenses(licenseIds, progress) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const result = yield this._axiosInstance.post(queryLicenseUrl, this._createReqBody(licenseIds));
                const data = result.data;
                const resultMap = new Map();
                if (data) {
                    for (const ohLicenseRspDto of data) {
                        const axiosResponse = yield this._axiosInstance.get(ohLicenseRspDto.url);
                        const licenseContent = axiosResponse.data;
